...
public class Display extends JFrame {
	...
	private static final int mapWidth = 1060;//1280
	private static final int mapHeight = 760;//820
	...
	Display() {
		...
		JTextArea textarea = new JTextArea("설명",19,14);
		JTextField textfield = new JTextField("배속 입력");
	...
	public void update(Graphics g) {
		...
		if(runScreen == false) {
			for (int i = 0; i < preySize; i++) {
				x = (int) (Math.random() * (mapWidth - 30)) + 10;
				y = (int) (Math.random() * (mapHeight - 30)) + 30;
				preys.add(new Prey(x, y));
			}
			for (int i = 0; i < predatorSize; i++) {
				x = (int) (Math.random() * (mapWidth - 30)) + 10;
